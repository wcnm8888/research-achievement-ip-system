...
const prepare = async (payload) => {
  ...
  const baseDepartment = await prisma.department.create({
    data: {
      code: `S69G_BASE_${suffix}`,
      name: "Step 69G Synthetic Base",
    },
  });
  const importDepartment = await prisma.department.create({
    data: {
      code: `S69G_DEPT_${suffix}`,
      name: "Step 69G Synthetic Import Department",
    },
  });
  const systemConfig = await ensurePermission("system:config");
  const adminRole = await createRole(`S69G_ADMIN_${suffix}`, [systemConfig.id]);
  const limitedRole = await createRole(`S69G_LIMITED_${suffix}`);
  const admin = await createUser({
    email: `step69g-admin-${lower}@example.invalid`,
    name: "Step 69G Admin",
    roleId: adminRole.id,
    departmentId: baseDepartment.id,
  });
  const limited = await createUser({
    email: `step69g-limited-${lower}@example.invalid`,
    name: "Step 69G Limited",
    roleId: limitedRole.id,
    departmentId: baseDepartment.id,
  });
  const ownerA = await createUser({
    email: `step69g-owner-a-${lower}@example.invalid`,
    name: "Step 69G Owner A",
    departmentId: importDepartment.id,
  });
  const ownerB = await createUser({
    email: `step69g-owner-b-${lower}@example.invalid`,
    name: "Step 69G Owner B",
    departmentId: importDepartment.id,
  });
  const contributorA = await createUser({
    email: `step69g-contributor-a-${lower}@example.invalid`,
    name: "Step 69G Contributor A",
    departmentId: importDepartment.id,
  });
  const contributorB = await createUser({
    email: `step69g-contributor-b-${lower}@example.invalid`,
    name: "Step 69G Contributor B",
    departmentId: importDepartment.id,
  });

  return {
    suffix,
    titlePrefix: `Step 69G Synthetic ${suffix}`,
...
const makeImportFile = (payload) => {
  ...
  return {
    originalName: payload.fileName ?? "step69g-achievements.csv",
    mimeType: "text/csv",
    size: buffer.length,
    buffer,
  };
...
const listAchievements = async (payload) => {
  const items = await prisma.achievement.findMany({
    where: {
      title: { startsWith: payload.titlePrefix ?? "Step 69G Synthetic" },
    },
...
const evidence = async (payload) => {
  const achievements = await prisma.achievement.findMany({
    where: {
      title: { startsWith: payload.titlePrefix },
    },
    select: {
      id: true,
      type: true,
  ...
  return {
    ...
    body: {
      ...
      softwareDraftCount: achievements.filter(
        (item) => item.type === "SOFTWARE_COPYRIGHT" && item.status === "DRAFT",
      ).length,
      softwareDetailCount: achievements.filter((item) => item.softwareCopyrightDetail)
        .length,
      normalizedRegistrationPersistedCount: achievements.filter((item) =>
        Boolean(item.softwareCopyrightDetail?.registrationNoNormalized),
      ).length,
...
try {
  ...
  if (!result) {
    throw new Error("Unsupported Step 69G helper action.");
  }
