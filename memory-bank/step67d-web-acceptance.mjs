...
const runBrowser = async (session, url) => {
  ...
  try {
    const output = await runAsync(
      "playwright-cli",
      [`-s=${session}`, "--raw", "run-code", "--filename=memory-bank/step67d-browser-acceptance.js"],
      { timeout: 180000 },
    );
    try {
...
const main = async () => {
  ...
  dockerCompose(["cp", "memory-bank/step67d-db-helper.mjs", "api:/app/step67d-db-helper.mjs"]);
