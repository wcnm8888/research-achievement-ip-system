...
- Status: DONE.
- Task classification:
  - S docs-only closure/archive update.
- Starting point:
  - HEAD at task start: `9bab53f docs: add phase one demo presenter brief`.
  - `git status --short` showed only existing untracked local artifacts.
  - `git diff --stat` and `git diff --cached --stat` were empty.
- Scope completed:
  - Added `memory-bank/phase-one-final-archive.md`.
  - Updated `memory-bank/project-requirement-completion-matrix.md` to reflect Step 83-Step 96 outcomes and explicitly mark mock/local/demo items as not real integration or production acceptance.
  - Updated `memory-bank/next-phase-options.md` so the default recommendation is now Route A rehearsal / Route B phase-two enhancement / Route C real integration preparation / Route D production readiness.
  - Updated `memory-bank/progress.md` and `memory-bank/evidence.md`.
...
- Status: DONE.
- Task classification:
  - XS/S docs-only link-in for the Step 79A `ImportJobItem` production
    read-only preflight runbook.
- Scope completed:
  - Linked `memory-bank/import-job-item-production-readonly-preflight-runbook.md`
    from `deploy/runbook-production.md`.
  - Linked the same runbook from `deploy/checklist-production-cutover.md`.
  - Distinguished the existing
    `memory-bank/import-job-history-production-readonly-preflight-runbook.md`
    as `ImportJob` / `ImportRun` aggregate history readiness, while the new
    runbook covers `ImportJobItem` row-level safe history readiness only.
  - Clarified that the `ImportJobItem` runbook is read-only reference material
    for migration-state, `import_job_items` table-structure, GET-only item API,
    response allowlist, and Web aggregate-only boundary checks.
  - Preserved evidence boundaries against recording `DATABASE_URL`, passwords,
...
- Explicitly not done:
  ...
  - No migration apply/deploy/reset.
  - No service startup.
  - No real import, DB write, Web row-level display, export, download, retry,
    delete, cleanup, or rollback.
  - No Web/backend runtime/code/test change.
  - No Prisma schema or migration change.
  - No package/lockfile/config change.
  - Existing untracked local artifacts were not touched.
- Verification:
  - Typecheck/test/build intentionally not run because this Step is docs-only
    and changes only documentation links and memory-bank records.
  - Diff checks and final status are recorded in `memory-bank/evidence.md`.
...
- Status: DONE.
- Task classification:
  - S-level backend writer slice; User account `CREATE_ONLY_PENDING_NO_CREDENTIAL` success path only, with no schema/migration, API, Web, database apply, production access, or real-data execution.
- Scope completed:
  - Added User account `ImportJobItem` writer inside `UserAccountImportJobRepository.markSucceededInTransaction`.
  - The writer runs in the existing business Prisma transaction shared by user creation, role creation, audit writes, and `ImportRun` / `ImportJob` success updates.
  - `UserAccountImportJobSuccessInput` item sub-input accepts only `rowNumber`, `safeCode`, and internal `targetId`; repository injects `jobId` and `runId` from the success input.
  - Successful User account apply builds item sub-input from applied in-memory rows only.
  - Item rows use fixed `plannedAction: CREATE_PENDING_USER`, `status: APPLIED`, `targetType: USER`, and `safeCode: null`.
  - Added repository tests proving `importJobItem.createMany` receives only allowlist data with consistent `jobId` / `runId`.
  - Added service tests proving success path passes item sub-inputs, and replay/in-progress/rejected/failed paths do not call the success writer.
  - Updated `memory-bank/import-job-history-database-model-plan.md`, `memory-bank/evidence.md`, and `memory-bank/decisions.md`.
- Explicitly not done:
  ...
  - No real import apply, real data write, seed, backfill, or fixture row.
  - No retry/delete/cleanup/rollback/download/export behavior.
  - No raw CSV, row values, account identifiers, credentials, invites, passwords, connection strings, `safeSummary`, or `auditLogIds` are written to item rows.
  - `targetId` remains internal-only and was not added to DTO/API/Web/log/evidence examples.
- Verification:
  - `corepack pnpm --filter @research-ip/api test -- user-account-import-job user-account-import-dry-run`: PASS; 4 files / 34 tests passed.
  - `corepack pnpm --filter @research-ip/api typecheck`: PASS.
  - `git diff --check`: PASS.
  - `git diff --cached --check`: PASS.
  - `git diff --stat`: PASS; tracked changes limited to User account import writer/service tests plus memory-bank docs.
  - `git diff --cached --stat`: PASS; empty before staging.
  - `git status --short`: PASS; tracked changes plus existing untracked local artifacts only.
  - Manual diff review: PASS; no schema/migration, API/Web/read DTO, DB apply, production access, row values, raw identifiers, CSV content, account identifiers, credentials, or `auditLogIds` in item data.

## 2026-07-05 Step 77C - Achievement ImportJobItem backend writer

- Status: DONE.
- Task classification:
  - S-level backend writer slice; Achievement `CREATE_DRAFT_ONLY` success path only for `PAPER`, `SOFTWARE_COPYRIGHT`, and `PATENT`, with no schema/migration, API, Web, database apply, production access, or real-data execution.
- Scope completed:
  - Added Achievement `ImportJobItem` writer inside `AchievementImportJobRepository.markSucceededInTransaction`.
  - The writer runs in the existing business Prisma transaction shared by Achievement creation, typed detail creation, contributor creation, audit writes, and `ImportRun` / `ImportJob` success updates.
  - `AchievementImportJobSuccessInput` item sub-input accepts only `rowNumber`, `safeCode`, and internal `targetId`; repository injects `jobId` and `runId` from the success input.
  - Successful Achievement apply builds item sub-input from applied in-memory rows only.
  - Item rows use fixed `plannedAction: CREATE_DRAFT`, `status: APPLIED`, `targetType: ACHIEVEMENT`, and `safeCode: null`.
  - Added repository tests proving `importJobItem.createMany` receives only allowlist data with consistent `jobId` / `runId`.
  - Added service tests proving `PAPER`, `SOFTWARE_COPYRIGHT`, and `PATENT` success paths pass item sub-inputs, and replay/in-progress/rejected/failed paths do not call the success writer.
  - Updated `memory-bank/import-job-history-database-model-plan.md`, `memory-bank/evidence.md`, and `memory-bank/decisions.md`.
- Explicitly not done:
  - No `prisma/schema.prisma` change and no migration file.
  - No migration apply/deploy/reset and no database access.
  - No User/account item writer.
  - No read DTO, API route/controller, client contract, or Web changes.
  - No production/VPS or production DB access.
  - No `.env` / `.env.production` content read.
  - No real import apply, real data write, seed, backfill, or fixture row.
  - No retry/delete/cleanup/rollback/download/export behavior.
  - No raw CSV, row values, personal identifiers, achievement identifiers, fee/reminder fields, credentials, connection strings, `safeSummary`, or `auditLogIds` are written to item rows.
  - `targetId` remains internal-only and was not added to DTO/API/Web/log/evidence examples.
- Verification:
  - `corepack pnpm --filter @research-ip/api test -- achievement-import-job achievement-import-dry-run`: PASS; 4 files / 50 tests passed.
  - `corepack pnpm --filter @research-ip/api typecheck`: PASS.
  - `git diff --check`: PASS.
  - `git diff --cached --check`: PASS.
  - `git diff --stat`: PASS; tracked changes limited to Achievement import writer/service tests plus memory-bank docs.
  - `git diff --cached --stat`: PASS; empty before staging.
  - `git status --short`: PASS; tracked changes plus existing untracked local artifacts only.
  - Manual diff review: PASS; no schema/migration, API/Web/read DTO, DB apply, production access, User writer, row values, raw identifiers, CSV content, personal/achievement identifiers, fee/reminder fields, credentials, or `auditLogIds` in item data.
...
- Status: DONE.
- Task classification:
  - S-level schema/migration-only implementation; no runtime writer, API, Web, database apply, production access, or real-data execution.
- Scope completed:
  - Added `ImportJobItemPlannedAction`, `ImportJobItemStatus`, and `ImportJobItemTargetType` to `prisma/schema.prisma`.
  - Added `ImportJobItem` model with only the Step 76A/76C/76D allowlist fields: `jobId`, `runId`, `rowNumber`, `plannedAction`, `status`, `safeCode`, `targetType`, and internal-only `targetId`.
  - Added `ImportJob.items` and `ImportRun.items` relation arrays.
  - Preserved conservative `onDelete: Restrict` on both item relations.
  - Added additive migration `prisma/migrations/20260705120000_add_import_job_items/migration.sql` creating item enums, `import_job_items`, indexes, and FKs only.
  - Generated Prisma Client locally.
  - Updated `memory-bank/import-job-history-database-model-plan.md`, `memory-bank/evidence.md`, and `memory-bank/decisions.md` for the schema/migration-only boundary.
- Explicitly not done:
  - No backend service/controller/repository/import writer changes.
  - No API route, DTO, client contract, or Web changes.
  - No migration apply/deploy/reset and no database access.
  - No production/VPS or production DB access.
  - No `.env` / `.env.production` content read.
  - No import apply, seed, backfill, fixture row, or real data write.
  - No retry/delete/cleanup/rollback/download/export behavior.
  - No JSON fields or raw CSV/source row/person/business identifier fields.
  - `targetId` remains internal-only persistence and was not added to any DTO/Web surface.
  - Existing untracked local artifacts were not touched.
- Verification:
  - `corepack pnpm prisma:validate`: initial no-env run failed before schema validation because `DATABASE_URL` was absent; rerun with a command-local non-secret dummy `DATABASE_URL` passed.
  - `corepack pnpm prisma generate`: PASS with command-local non-secret dummy `DATABASE_URL`.
  - `corepack pnpm --filter @research-ip/api typecheck`: PASS.
  - `git diff --check`: PASS.
  - `git diff --stat`: PASS; tracked changes limited to Prisma schema, additive migration, and memory-bank docs.
  - `git diff --cached --stat`: PASS; empty before staging.
  - `git status --short`: PASS; tracked changes plus existing untracked local artifacts only.
  - Manual diff review: PASS; migration is additive, no DB apply/reset, no seed/backfill, no API/Web/writer change, no raw CSV/personal identifier/sensitive values/credentials, and `targetId` did not enter DTO/Web.
...
- Status: DONE.
- Task classification:
  - S-level docs-only schema planning task; no schema, migration, runtime, API, Web, database, or operational execution.
- Scope completed:
  - Added `memory-bank/import-job-item-schema-plan.md`.
  - Designed a future `ImportJobItem` relationship model as child safe outcome rows under `ImportJob` and `ImportRun`.
  - Kept candidate fields limited to the Step 76A allowlist: `jobId`, `runId`, `rowNumber`, `plannedAction`, `status`, `safeCode`, `targetType`, and internal-only `targetId`.
  - Proposed enum candidates for planned action, item status, and a narrow import-item target type.
  - Recorded that a narrow `ImportJobItemTargetType` is preferred over broad reusable business-object target types to avoid drilldown semantics.
  - Planned indexes and constraints for `jobId`, `runId`, indexed `jobId + rowNumber`, composite `runId + rowNumber` table identity, and optional `status` / `safeCode` diagnostics.
  - Required conservative `onDelete: Restrict` relation behavior and explicitly rejected cascade deletion of history.
  ...
  - Reconfirmed `targetId` is internal-only and must not enter Web DTOs.
  - Reconfirmed JSON fields must not be used to store row values.
  - Split future implementation into schema/migration, backend writer, backend read DTO, Web plan, and acceptance steps.
  - Updated `memory-bank/import-job-history-database-model-plan.md` with the Step 76C plan addendum.
  - Updated `memory-bank/decisions.md` with the docs-only schema planning decision.
  - Updated `memory-bank/evidence.md` with docs-only verification evidence.
- Explicitly not done:
  - No `prisma/schema.prisma` change.
  - No migration generation.
  - No runtime/API/Web/package/lockfile/config changes.
  - No Web/API/Docker/browser startup.
  - No database, production/VPS, or production DB access.
  - No `.env` / `.env.production` content read.
  - No backend writer, backend read DTO, Web implementation, migration execution, import apply, real-data import, retry, delete, cleanup, rollback, download, export, row-level API, row-level Web display, `targetId` display, or business-object drilldown.
  - No typecheck/test/build because this Step changed documentation only.
- Verification:
  - `git diff --check`: PASS.
...
- Status: DONE.
- Scope completed:
  - Added `memory-bank/step72i-software-import-job-acceptance.mjs`.
  - Reused the local Docker API/Postgres environment for production-like API/DB acceptance.
  - Rebuilt the local API image, updated the local API container, confirmed the container contained the Step 72H compiled achievement import job repository, and confirmed existing Prisma migrations had no pending local Docker DB migration.
  - Ran the Step 72I synthetic helper inside the local API container.
- Acceptance result:
  - Success path: PASS, first `SOFTWARE_COPYRIGHT` `CREATE_DRAFT_ONLY` apply returned `EXECUTED`, created 2 draft software copyright achievements, 2 software copyright details, 2 contributors, 2 audit rows, and persisted `ImportJob.status = SUCCESS` plus `ImportRun.status = SUCCESS`.
  - `SUCCESS` replay: PASS, returned `REPLAYED_SUCCESS`, created no additional achievement/detail/contributor/audit/job/run rows, and returned stored safe counts.
  - `REJECTED` replay: PASS, missing registration validation returned safe `REQUIRED` rejection, created no achievement/detail/contributor/audit rows, and kept one rejected job/run across replay.
  - `RUNNING` in-flight: PASS, DB-helper-seeded same-key running claim returned `IMPORT_IN_PROGRESS`, created no achievement/detail/contributor/audit rows, and kept one running job/run.
  - Safe summary scan: PASS.
  - Side-effect boundary: PASS.
  - PATENT import job check: PASS, no `PATENT` job/idempotency path was exercised.
- Verification:
  - `docker build -f Dockerfile.api -t research-achievement-production-api .`: PASS.
  - `docker compose -f docker-compose.production.yml up -d --no-deps api`: PASS; Docker reported existing orphan containers but no orphan cleanup was run.
  - Local API container health check after update: PASS.
  - Container DB environment presence was checked as a boolean only; the value was not printed or recorded.
  - `docker exec research-achievement-production-api-1 sh -lc 'corepack pnpm prisma migrate deploy'`: PASS, no pending migrations.
  - `docker exec research-achievement-production-api-1 sh -lc 'node /app/memory-bank/step72i-software-import-job-acceptance.mjs'`: PASS.
  - `corepack pnpm --filter @research-ip/api test -- achievement-import department-import imports.app-module`: PASS, 9 files / 78 tests.
  - `corepack pnpm --filter @research-ip/api typecheck`: PASS.
- Explicitly not done:
  - No Web changes.
  - No `PATENT` or user/account import wiring.
  - No schema or migration changes.
  - No production/VPS access, production DB/config access, `.env` / `.env.production` content read, real-data import, Docker orphan cleanup, prune, volume deletion, file deletion, reset, restore, checkout, or known unrelated untracked local artifact handling.
  - No `DATABASE_URL` value, credential, password, token, cookie, connection string, raw CSV, registration number, normalized registration number, title, contributor/owner email/name, or raw path was printed or recorded.
...
- Status: DONE.
- Scope completed:
  - Added `apps/api/src/imports/achievement-import-job.repository.ts`.
  - Added `apps/api/src/imports/achievement-import-job.repository.spec.ts`.
  - Updated `apps/api/src/imports/achievement-import-dry-run.service.ts`.
  - Updated `apps/api/src/imports/achievement-import-dry-run.service.spec.ts`.
  - Updated `apps/api/src/imports/imports.module.ts`.
  - Updated `memory-bank/import-job-history-database-model-plan.md`.
  - Updated `memory-bank/evidence.md`.
- Key outcome:
  - Achievement `PAPER` `CREATE_DRAFT_ONLY` apply now computes server-side idempotency from family `ACHIEVEMENT`, mode `CREATE_DRAFT_ONLY`, achievement type `PAPER`, SHA-256 file fingerprint, safe target environment discriminator, scope type, and scope hash.
  - First same-key PAPER apply creates `ImportJob` + `ImportRun` in `RUNNING`.
  - Same-key `SUCCESS` returns safe `REPLAYED_SUCCESS` without opening the business transaction.
  - Same-key `RUNNING` returns `IMPORT_IN_PROGRESS` without opening the business transaction.
  - Same-key `REJECTED` returns stored safe rejection through the existing rejected error path.
  - Same-key `FAILED` blocks automatic retry.
  - Successful PAPER apply keeps achievement creation, `PaperDetail`, contributors, audit writes, `ImportRun` success summary, and `ImportJob` success summary in one Prisma transaction.
  - Validation-blocked PAPER apply stores safe `REJECTED` summary without writing achievement/detail/contributor/audit rows.
  - Persisted safe summaries store only safe counts, operation/status/code fields, and do not store raw CSV, raw/normalized DOI, title, abstract, owner/contributor email/name, raw path, credential/session/token/cookie/password/connection string/env/storage/mail payload values.
  - `SOFTWARE_COPYRIGHT` and `PATENT` apply behavior was left on the existing non-job path.
- Explicitly not done:
  - No Web changes.
  - No `SOFTWARE_COPYRIGHT` import job/idempotency wiring.
  - No `PATENT` import job/idempotency wiring.
  - No user/account import idempotency wiring.
  - No schema or migration changes.
  - No apply API execution.
...
- Status: DONE.
- Scope completed:
  - Extended Web achievement import apply from `PAPER`-only to homogeneous all-`PAPER` or all-`SOFTWARE_COPYRIGHT` batches.
  - Added Web result typing for `SOFTWARE_COPYRIGHT` apply rows and `createdSoftwareCopyrightDetailsCount`.
  - Added type-aware apply eligibility with `applyType: "PAPER" | "SOFTWARE_COPYRIGHT" | null`.
  - Kept mixed `PAPER` + `SOFTWARE_COPYRIGHT`, `PATENT`, missing DOI, missing software registration number, dry-run error/warning/`DB_CONFLICT`, non-`CREATE_DRAFT`, stale fingerprint, and in-flight requests disabled before confirmation.
  - Updated confirmation copy and success display to be type-aware while keeping `CREATE_DRAFT_ONLY`, `DRAFT` only, no approval submission, no workflow, no attachment/storage, no fee/reminder, no notification/search/resource grant, and no import job boundaries.
  - Kept rejected apply display limited to safe counts and safe error codes.
  - Added implementation addendum to `memory-bank/achievement-import-software-web-entry-design.md`.
- Explicitly not done:
  - No backend/API implementation.
  - No Prisma schema/migration change.
