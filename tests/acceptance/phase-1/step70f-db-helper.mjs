...
const forbidden = () => ({
  status: 403,
  body: { message: "Missing required permission: system:config." },
});
...
const prepare = async (payload) => {
  ...
  const baseDepartment = await prisma.department.create({
    data: {
      code: `S70F_BASE_${suffix}`,
      name: "Step 70F Synthetic Base",
    },
  });
  const importDepartment = await prisma.department.create({
    data: {
      code: `S70F_DEPT_${suffix}`,
      name: "Step 70F Synthetic Import Department",
    },
  });
  const systemConfig = await ensurePermission("system:config");
  const adminRole = await createRole(`S70F_ADMIN_${suffix}`, [systemConfig.id]);
  const limitedRole = await createRole(`S70F_LIMITED_${suffix}`);
  const admin = await createUser({
    email: `step70f-admin-${lower}@example.invalid`,
    name: "Step 70F Admin",
    roleId: adminRole.id,
    departmentId: baseDepartment.id,
  });
  const limited = await createUser({
    email: `step70f-limited-${lower}@example.invalid`,
    name: "Step 70F Limited",
    roleId: limitedRole.id,
    departmentId: baseDepartment.id,
  });
  const ownerA = await createUser({
    email: `step70f-owner-a-${lower}@example.invalid`,
    name: "Step 70F Owner A",
    departmentId: importDepartment.id,
  });
  const ownerB = await createUser({
    email: `step70f-owner-b-${lower}@example.invalid`,
    name: "Step 70F Owner B",
    departmentId: importDepartment.id,
  });
  const contributorA = await createUser({
    email: `step70f-contributor-a-${lower}@example.invalid`,
    name: "Step 70F Contributor A",
    departmentId: importDepartment.id,
  });
  const contributorB = await createUser({
    email: `step70f-contributor-b-${lower}@example.invalid`,
    name: "Step 70F Contributor B",
    departmentId: importDepartment.id,
  });

  return {
    suffix,
    titlePrefix: `Step 70F Synthetic ${suffix}`,
...
const makeImportFile = (payload) => {
  ...
  return {
    originalName: payload.fileName ?? "step70f-achievements.csv",
    mimeType: "text/csv",
    size: buffer.length,
    buffer,
  };
...
const listAchievements = async (payload) => {
  const items = await prisma.achievement.findMany({
    where: { title: { startsWith: payload.titlePrefix ?? "Step 70F Synthetic" } },
...
const optionalCount = async (modelName) =>
  prisma[modelName]?.count ? await prisma[modelName].count() : 0;
...
const countForbiddenSideEffects = async () => ({
  ...
  importJobCount: await optionalCount("importJob"),
});

const evidence = async (payload) => {
  const achievements = await prisma.achievement.findMany({
    where: { title: { startsWith: payload.titlePrefix } },
    select: {
      id: true,
      type: true,
      status: true,
  ...
  return {
    ...
    body: {
      ...
      patentDraftCount: achievements.filter(
        (item) => item.type === "PATENT" && item.status === "DRAFT",
      ).length,
      patentDetailCount: achievements.filter((item) => item.patentDetail).length,
      normalizedApplicationPersistedCount: achievements.filter((item) =>
        Boolean(item.patentDetail?.applicationNoNormalized),
      ).length,
      normalizedGrantPersistedCount: achievements.filter((item) =>
        Boolean(item.patentDetail?.grantNoNormalized),
      ).length,
      nextFeeDatePersistedCount: achievements.filter((item) =>
        Boolean(item.patentDetail?.nextFeeDate),
      ).length,
      feeAmountPersistedCount: achievements.filter((item) =>
        Boolean(item.patentDetail?.feeAmount),
      ).length,
...
try {
  ...
  if (!result) {
    throw new Error("Unsupported Step 70F helper action.");
  }
