...
const runBrowser = async (session, url) => {
  ...
  try {
    const output = await runAsync(
      "playwright-cli",
      [`-s=${session}`, "--raw", "run-code", "--filename=tests/acceptance/phase-1/step65f-browser-acceptance.js"],
      { timeout: 120000 },
    );
    try {
      return JSON.parse(output);
...
const main = async () => {
  ...
  dockerCompose(["cp", "tests/acceptance/phase-1/step65f-db-helper.mjs", "api:/app/step65f-db-helper.mjs"]);
