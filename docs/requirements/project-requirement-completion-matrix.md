...
| ImportJobItem 后端只读 API | 已完成 | Step 78B/D，`GET /api/import-jobs/:id/items`；Step 112 Web 只接入该既有 backend-only API | 仍不代表 retry/export/raw JSON/rollback/drilldown 或 production import acceptance |
| ImportJobItem Web 行级安全只读展示 | 已完成本地闭环 | Step 111 方案；Step 112 Web API client/types/detail panel；Step 113 localhost/local-demo/synthetic UI acceptance 与 closure | 只在 ImportJob detail 内展示 rowNumber/plannedAction/status/safeCode/targetType；禁止 targetId、raw payload、raw JSON、safeSummary item/debug 字段、auditLogIds、jobId/runId 明细、retry/export/rollback/drilldown |
...
| 成果转化深化 MVP | 已完成本地演示闭环 | Step 107-B-C 技术方案；Step 108-B-C additive schema/backend；Step 109-B-C Web 展示/编辑；Step 110-B-C localhost/local-demo/synthetic UI acceptance 和闭环归档 | 仅为本地深化切片；不等于真实合同、法务、财务、付款、发票、结算、生产监控、生产验收或真实外部系统联调 |
| 成果转化全流程 | 本地深化 MVP 已闭环，真实全流程未完成 | 原始需求列为二期深化；Step 84 local ledger 和 Step 107-110 本地深化 MVP 已完成 | 后续若要做真实全流程，必须另立真实合同/法务/财务/付款/发票/结算/外部系统 readiness 与授权方案 |
