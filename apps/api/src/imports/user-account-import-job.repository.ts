...
export type UserAccountImportJobClaimInput = {
...
export type UserAccountImportJobRunnerClaim = {
  disposition: "RUNNER";
  jobId: string;
  runId: string;
};

export type UserAccountImportJobExistingClaim = {
...
export type UserAccountImportJobClaimResult =
  | UserAccountImportJobRunnerClaim
...
export type UserAccountImportJobFailedInput = {
  jobId: string;
  runId: string;
  failureCode: string;
...
@Injectable()
export class UserAccountImportJobRepository {
  constructor(@Inject(PrismaService) private readonly prisma: PrismaService) {}

  async claimUserAccountPendingNoCredentialJob(
    input: UserAccountImportJobClaimInput,
  ): Promise<UserAccountImportJobClaimResult> {
    try {
      return await this.prisma.$transaction(async (tx) => {
        const client = tx as UserAccountImportJobTransactionClient;
        const job = await client.importJob.create({
          data: {
            idempotencyKeyHash: input.idempotencyKeyHash,
            importFamily: ImportFamily.USER_ACCOUNT,
  ...
  async markSucceededInTransaction(
    client: UserAccountImportJobTransactionClient,
    ...
    await client.importJob.update({
      ...
      data: {
        ...
        createdBusinessCount: input.createdUsersCount,
        createdCompanionCount: input.createdUserRolesCount,
  ...
  async markRejected(input: UserAccountImportJobRejectedInput): Promise<void> {
    const now = new Date();
  ...
  async markFailed(input: UserAccountImportJobFailedInput): Promise<void> {
    const now = new Date();
    await this.prisma.$transaction(async (tx) => {
      const client = tx as UserAccountImportJobTransactionClient;
      await client.importRun.update({
  ...
  private async findExistingClaim(
    input: UserAccountImportJobClaimInput,
  ): Promise<UserAccountImportJobExistingClaim> {
...
): UserAccountImportJobExistingClaim["disposition"] => {
  switch (status) {
