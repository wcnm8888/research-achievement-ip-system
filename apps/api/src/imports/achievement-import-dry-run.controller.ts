...
@Controller("achievements/import")
@UseGuards(UserContextGuard, PermissionGuard)
export class AchievementImportDryRunController {
  constructor(
    @Inject(AchievementImportDryRunService)
    private readonly achievementImportDryRunService: AchievementImportDryRunService,
  ) {}
  ...
  async dryRunAchievementImport(
    @CurrentUser() currentUser: UserContext,
    @UploadedFile() file?: UploadedImportFile,
  ) {
    try {
      return await this.achievementImportDryRunService.dryRunAchievementCsv(
        currentUser,
...
const toAchievementImportDryRunFile = (
...
const mapAchievementImportDryRunError = (error: unknown): Error => {
  if (
  ...
  if (error instanceof InvalidAchievementImportCsvError) {
